...
public:
    ...
    */
    ...
    void drawConvexPolygons(const GPoint points[], int count, const GPaint& paint) {
        ...
        float min = edges.at(start).top;

        for (int y = min; y < max; y++) {
            Edge left = edges.at(start);
            Edge right = edges.at(stop);

            blit(left.calculateX(y), right.calculateX(y), y, paint);
            if (left.isValid(y + 1) == false) {
                start = index;
                index++;
                if (start >= edges.size()) {
                    return;
